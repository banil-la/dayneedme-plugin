...
const UrlShare: React.FC<UrlShareProps> = ({ onUpdateRecentUrls }) => {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      switch (message.type) {
        case "URL_SELECTION_CHANGED":
          // console.log("[UrlShare] URL selection changed:", message.data);
          setIsFrameSelected(!!message.data);
          break;
        case "SHARE_LINK_RECEIVED":
          if (message.link) {
            handleCreateShortUrl(message.link, message.description);
          } else {
